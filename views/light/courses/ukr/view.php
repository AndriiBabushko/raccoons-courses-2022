    ...
    <div class="row my-2">
        <?php if (empty($videos)): ?>
            <div class="col-12">
                <div class="alert alert-warning my-2 w-100 text-center">
                    ...
                    На цьому курсі ще немає відео! Перевірте пізніше :)
                </div>
            </div>
        ...
        <?php if (!$courseAccess && !$isCourseMaker): ?>
            <div class="col-12">
                <div class="alert alert-warning my-2 w-100 text-center">
                    <i class="bi bi-exclamation-diamond-fill"></i>
                    Придбайте курс для повного доступу!
                </div>
            </div>
        <?php endif; ?>
                        ...
                        <?php if ($courseAccess || $isCourseMaker): ?>
                            <?php foreach ($videos as $video): ?>
                                <li class="nav-item w-100 settings-item">
                                    <a href="/courses/ukr/view/<?php echo $idGood; ?>?id_video=<?php echo $video['id_video'] ?>"
    ...
    <?php if (!empty($isCourseMaker) && $isCourseMaker): ?>
        <div class="row mt-2">
            <div class="col-12 col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-4 mb-2">
                <a class="btn btn-success w-100" href="/video/ukr/add?id_good=<?php echo $idGood; ?>">
            ...
            <div class="col-12 col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-4 mb-2">
                <a class="btn btn-primary w-100" href="/video/ukr/update?id_good=<?php echo $idGood; ?>">
            ...
            <div class="col-12 col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-4 mb-2">
                <a class="btn btn-danger w-100" href="/video/ukr/delete?id_good=<?php echo $idGood; ?>">
                    ...
                    <div class="comment-block border-2">
                        <div class="comment-form-container">
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    <label for="comment" id="labelComment" class="form-label label">Залишити коментар</label>
                                    <textarea class="input form-control short" name="comment" id="comment"
                                              placeholder="Залиште коментар тут :)"><?php if (!empty($model)) echo $model['comment']; ?></textarea>
                                </div>

                                <div class="form-group mt-2">
                                    <button type='submit' id='buttonSendComment' class='btn btn-secondary w-100'>Відправити коментар</button>
                                </div>
                            </form>
                        </div>
                        ...
                        <?php if (empty($comments)): ?>
                            <div class="alert alert-dark m-2 text-center w-100">
                                <i class="bi bi-exclamation-diamond-fill"></i>
                                Коментарів до курсу поки немає! Поверніться пізніше або напишіть свій :>
                            </div>
                        <?php endif; ?>
                                            ...
                                            <?php if ($isCourseMaker || $currentUser['id_user'] == $comment['id_user']): ?>
                                                <div class="d-flex align-items-center">
                                                    <a class="btn btn-danger"
                                                       href="/courses/ukr/deleteComment?id_comment=<?php echo $comment['id_comment']; ?>&id_good=<?php echo $idGood; ?>"
                                                    >
                                                        <i class="bi bi-trash"></i>
                                                    </a>
                                                </div>
